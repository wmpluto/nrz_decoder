...
class Decoder(srd.Decoder):
    ...
    outputs = ['nrz']
    channels = (
        {'id': 'stream', 'type': 209, 'name': 'Stream', 'desc': 'UART transceive line'},
    )
    options = (
        {'id': 'baudrate', 'desc': 'Baud rate', 'default': 9600},
        {'id': 'code', 'desc': 'Coding style', 'default': 'nrz',
            'values': ('nrz', 'man')},
        {'id': 'header', 'desc': 'Header between WUP and Data', 'default': '1001'},    
        {'id': 'invert', 'desc': 'Invert Signal?', 'default': 'no',
            'values': ('yes', 'no')},
        {'id': 'bit_order', 'desc': 'Bit order', 'default': 'lsb-first',
            'values': ('lsb-first', 'msb-first')},
        {'id': 'marginpct', 'desc': 'Error margin %', 'default': 10,
            'values': (5, 10, 15, 20, 25)},
        {'id': 'stop sequence', 'desc': 'Stop bits', 'default': '0000'},
    )
    annotations = (
        ('108', 'data', 'data'),
        ('107', 'data-bits', 'data bits'),
    ...
    def decode(self, ss, es, logic):
        for (self.samplenum, pins) in logic:
            ...
            # Save current Stream values for the next round.
            self.oldbit = signal
